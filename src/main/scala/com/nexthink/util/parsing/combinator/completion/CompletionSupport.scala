...
/** `CompletionSupport` adds completion capability to parsers.
  ...
  *  [[com.nexthink.util.parsing.combinator.completion.CompletionSupport.Parser#completions]]
  *  which returns a instance of [[com.nexthink.util.parsing.combinator.completion.CompletionTypes.Completions]]
  *  for a certain input.
  *
  *  Combinators are overloaded to cover the additional completion aspect, so that no change is required in the grammar.
  *
  *  Note that the derived trait [[com.nexthink.util.parsing.combinator.completion.RegexCompletionSupport]] can be mixed-in
...
trait CompletionSupport extends Parsers with CompletionTypes {
  ...
  abstract class Parser[+T] extends super.Parser[T] {
    ...
    /** An unspecified method that defines the possible completions for this parser
      *
      * @param in the input
      * @return an instance of [[com.nexthink.util.parsing.combinator.completion.CompletionTypes.Completions]]
      */
